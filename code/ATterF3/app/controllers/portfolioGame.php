...
class portfolioGame extends Controller {
    ...
    public function index()
    {
        ...
        if (!isset($_SESSION['user_id'])) {
            ...
            $this->view('admin/portfolioGame', $data);
            $this->view('templates/admin/footer', $data);
        }
    }
    public function tambah()
    {
        if ($this->model("$this->model_name", 'Home_models')->tambahDataGame($_POST) > 0) {
            Flasher::setFlash('berhasil', 'ditambahkan', 'success');
            header('Location: ' . BASEURL . '/portfolioGame');
            exit;
        } else {
    ...
    {
        if ($this->model("$this->model_name", 'Home_models')->hapusDataGame($id) > 0) {
    ...
    {
        echo json_encode($this->model("$this->model_name", 'Home_models')->getGameById($_POST['id']));
    }

    public function ubah()
    {
        if ($this->model("$this->model_name", 'Home_models')->ubahDataGame($_POST) > 0) {
